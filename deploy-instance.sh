...
if [ "$NEEDS_ENV" = true ]; then
    ...
    if [ "$create_env" = true ]; then
        ...
        echo -e "${CYAN}에디터가 열립니다. 환경변수를 입력하세요.${NC}"
        echo ""
...
DATABASE_URL=postgresql://postgres:your_db_password@POSTGRES_PRIVATE_IP:5432/lecun2
...
CORS_ALLOWED_ORIGINS=http://NGINX_PUBLIC_IP
EOL
                ;;
            websocket)
                cat > .env << 'EOL'
# WebSocket 환경변수 (Backend와 동일하게!)

DEBUG=False
SECRET_KEY=your-django-secret-key-here-SAME-AS-BACKEND
...
AIRFLOW_POSTGRES_PASSWORD=airflow_db_password
AIRFLOW_FERNET_KEY=your_fernet_key_here
AIRFLOW_WEBSERVER_SECRET_KEY=your_webserver_secret_key
